...
class Station():
    ...
    def __init__(self):
        ...
        # self.ap_station.config(
        #     essid=self.database.read(self.access_point)[0],
        #     authmode=AUTH_WPA_WPA2_PSK,
        #     password=self.database.read(self.access_point)[1],
        #     max_clients=self.database.read(self.access_point)[2])
    ...
    def create_database(self, key):
        if key == self.known_networks:
            ...
            self.database.write(self.access_point, ["MicroPython", "MicropythoN", 5])
    ...
    def set_access_point(self, ap_ssid, ap_password, max_clients):
        access_point_list = []
        access_point_list.append(ap_ssid)
        access_point_list.append(ap_password)
        access_point_list.append(max_clients)
        self.database.write(self.access_point, access_point_list)
        print("AP configured with name: '{}' | Max Clients Allowed: {}".format(ap_ssid, max_clients))
        print("Reset the board to apply changes.")
    ...
    def _ui_ap_station(self):
        ...
        try:
            inp3 = int(input(self.prompt))

        except ValueError:
            pass

        self.set_access_point(inp1, inp2, inp3)
